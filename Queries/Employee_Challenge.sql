...
--Mentorship title count
SELECT COUNT(emp_no) as "mentorship_eligible_employees"
FROM mentorship_elgibility;

-- All Employee Count table
SELECT DISTINCT ON (e.emp_no) e.emp_no, e.first_name, e.last_name, e.birth_date,
		ttl.from_date, ttl.to_date, ttl.title
INTO all_employees
FROM employees e
...
-- Count of all employees not retiring or in mentorship
SELECT COUNT(emp_no) "employee_count"
FROM all_employees
WHERE emp_no NOT IN(SELECT emp_no FROM mentorship_elgibility)
AND emp_no NOT IN (SELECT emp_no FROM unique_titles);
